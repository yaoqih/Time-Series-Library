...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Simple forecasting head to map reconstructed window to future horizon.
        self.forecast_head = nn.Linear(self.seq_len, self.pred_len)
    ...
    def forecast(self, x_enc):
        ...
        x_input = rearrange(x_enc, "B L D -> (B D) L")
        enc_out = self.enc(x_input)
        # map reconstructed window to future horizon
        pred = self.forecast_head(enc_out)
        # [B * D, pred_len] -> [B, pred_len, D]
        dec_out = rearrange(pred, "(B D) L -> B L D", B=x_enc.size(0))
        return dec_out
