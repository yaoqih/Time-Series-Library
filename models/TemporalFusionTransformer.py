...
def _get_datatype(configs):
    if configs.data in datatype_dict:
        return datatype_dict[configs.data]
    observed_len = getattr(configs, 'enc_in', 0)
    if observed_len <= 0:
        raise ValueError(f"TFT requires enc_in > 0 for data='{configs.data}'")
    # Fallback: treat all input channels as observed, no static features.
    return TypePos([], list(range(observed_len)))
...
class TFTEmbedding(nn.Module):
    def __init__(self, configs):
        ...
        dtype = _get_datatype(configs)
        self.static_pos = dtype.static
        self.observed_pos = dtype.observed
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Number of variables
        dtype = _get_datatype(configs)
        self.static_len = len(dtype.static)
        self.observed_len = len(dtype.observed)
