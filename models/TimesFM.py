...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        if not self._available or self.model is None:
            last_val = x_enc[:, -1:, :].repeat(1, self.pred_len, 1)
            return last_val
        ...
        if hasattr(self.model, "forecast") and "horizon" in self.model.forecast.__code__.co_varnames:
            ...
            output, _ = self.model.forecast(
                inputs=[arr for arr in x_enc.cpu().numpy()],
                freq=[0] * x_enc.shape[0],
            )
