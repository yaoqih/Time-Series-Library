...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if x_mark_dec is not None and x_mark_dec.size(1) != seasonal_init_dec.size(1):
            # MICN expects seq_len + pred_len time features. Build from encoder + prediction marks.
            if x_mark_enc is not None and x_mark_enc.size(1) == self.seq_len and x_mark_dec.size(1) >= self.pred_len:
                x_mark_dec = torch.cat([x_mark_enc, x_mark_dec[:, -self.pred_len:, :]], dim=1)
            elif x_mark_dec.size(1) > seasonal_init_dec.size(1):
                x_mark_dec = x_mark_dec[:, -seasonal_init_dec.size(1):, :]
            else:
                pad_len = seasonal_init_dec.size(1) - x_mark_dec.size(1)
                pad = x_mark_dec[:, -1:, :].repeat(1, pad_len, 1)
                x_mark_dec = torch.cat([x_mark_dec, pad], dim=1)
