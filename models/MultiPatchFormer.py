...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Pad to ensure all patch scales produce the same number of patches.
        def _calc_pad(seq_len, kernel, stride, target_patches):
            return max(0, (target_patches - 1) * stride + kernel - seq_len)

        pad1 = _calc_pad(self.seq_len, self.patch_len1, self.stride1, self.patch_num1)
        pad2 = _calc_pad(self.seq_len, self.patch_len2, self.stride2, self.patch_num1)
        pad3 = _calc_pad(self.seq_len, self.patch_len3, self.stride3, self.patch_num1)
        pad4 = _calc_pad(self.seq_len, self.patch_len4, self.stride4, self.patch_num1)

        self.padding_patch_layer1 = nn.ReplicationPad1d((0, pad1))
        self.padding_patch_layer2 = nn.ReplicationPad1d((0, pad2))
        self.padding_patch_layer3 = nn.ReplicationPad1d((0, pad3))
        self.padding_patch_layer4 = nn.ReplicationPad1d((0, pad4))
        ...
        self.pred_splits = [self.pred_len // 8] * 7
        self.pred_splits.append(self.pred_len - sum(self.pred_splits))
        s1, s2, s3, s4, s5, s6, s7, s8 = self.pred_splits
        self.out_linear_1 = torch.nn.Linear(self.d_model, s1)
        self.out_linear_2 = torch.nn.Linear(self.d_model + s1, s2)
        self.out_linear_3 = torch.nn.Linear(self.d_model + s1 + s2, s3)
        self.out_linear_4 = torch.nn.Linear(self.d_model + s1 + s2 + s3, s4)
        self.out_linear_5 = torch.nn.Linear(self.d_model + s1 + s2 + s3 + s4, s5)
        self.out_linear_6 = torch.nn.Linear(self.d_model + s1 + s2 + s3 + s4 + s5, s6)
        self.out_linear_7 = torch.nn.Linear(
            self.d_model + s1 + s2 + s3 + s4 + s5 + s6, s7
        )
        self.out_linear_8 = torch.nn.Linear(
            self.d_model + s1 + s2 + s3 + s4 + s5 + s6 + s7, s8
        )
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        x_i_p1 = self.padding_patch_layer1(x_i)
